document.addEventListener("DOMContentLoaded", function() {
    ...
    function startTypeWriter(classList, prefix) {
        ...
        setTimeout(() => {
            ...
            if (classList === "intro-f") {
                ...
                text.id = 'dialog';
                textContent = `I know that I am just scratching the surface in web design, frontend/backend services and software development in general, yet that is what excites me the most about the future. I have interests in the software development and client delivery of services to predict stock market dynamics in real-time, services in FinTech, healthcare, global distribution systems, traffic prediction. I want to learn more about microservices, systems engineering, data science, data engineering, and database management. Software development in general is a giant ocean to me and I just want to go swim in it. I plan to certify myself on Coursera on machine learning, finish my certification at Full Stack Open and dive deeper into pattern recognition, deep learning, and sentiment analysis to name a few more rocks that I’d like to turn over. To be frank, I’m interested in working in any sort of data driven technology or industry.`;
    ...
    bottomRow.addEventListener("mouseout", function(event) {

        //bottomRow.style.borderWidth = `calc(var(--border-grids)) 0 0 0`;

        grid32 = document.querySelectorAll(".grid3x2__cell");

        // check if the mouse is outside the wrapper
        if ((!bottomRow.contains(event.relatedTarget) || event.target.classList.contains('')) && (deltaY > 0)) {
                ...
                integralIntroWrapper.classList.remove('open');
                integralContainer.style.display = "none";
                ...
                document.documentElement.style.setProperty('--animation-integral', `colorOut ${colorDuration}s ease-in`);

                document.documentElement.style.setProperty('--grid32-tran', '');
                ...
                links.forEach((link) => {
                    if (!link.id === `home` || !link.id === `simulate`) {
                        link.style.color = 'rgba(253, 181, 21, 1)';
                    }
                })

                document.documentElement.style.setProperty('--animation-links0', '');
                document.documentElement.style.setProperty('--animation-links2', '');
                document.documentElement.style.setProperty('--animation-links4', '');

                isPaused = true;
                savedDrops = [...drops];
                cancelAnimationFrame(animationFrameId);

                //document.getElementById('intro-row').scrollIntoView();
    ...
    bottomRow.addEventListener("scroll", function(event) {
        ...
        // check if the mouse is outside the wrapper
        if (!bottomRow.contains(event.relatedTarget) || event.target.classList.contains('')) {
            //dialogWrapper.style.display = "none";
            integralIntroWrapper.classList.remove('open');
            integralContainer.style.display = "none";
            ...
            document.getElementById('top-row').scrollIntoView();

            /* const myDiv = document.getElementById('mid-row'); // Replace 'myDivId' with the actual ID of your div
