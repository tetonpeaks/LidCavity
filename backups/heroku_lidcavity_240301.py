    ...
    if (flag == True):
        if (chk['chk'] == True): # change
            ...
            for row in rows:
                ...
                msg = f"{simname}_{user_id} updated."
                ctr += 1
            #print(f"ctr: {ctr}")
        else:
...
def login():
    ...
    #print(
    #        f"{bcolors.OKCYAN}user_id{bcolors.ENDC}: {user_id}, "
    #        f"{bcolors.OKCYAN}username{bcolors.ENDC}: {username}, "
    #        f"{bcolors.OKCYAN}user_login_status{bcolors.ENDC}: {user_login_status}, "
    #        f"{bcolors.OKCYAN}current_user.is_authenticated: {bcolors.ENDC}{current_user.is_authenticated}"
    #    )
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        #flash('', '')
        if user and user.check_password(password) and user.is_verified == True:
            ...
            flash('Invalid username and/or password or incomplete verification process', 'danger')
            return redirect(url_for('login'))
...
def verify(token, expiration_time):
    try:
        ...
        if user_id_from_url is not None:
            ...
            #if user and bcrypt.checkpw(hashed_token.encode('utf-8'), user.hashed_token.encode('utf-8')):
                # Update the user's verification status in the database (your verification logic here)
                # user.is_verified = True
                # db.session.commit()
...
def send_verification_email(user_id, email, username, token, expiration_time_str):
...
def register():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        send_verification_email(user_id, email, username, encrypted_token, encrypted_expiration_time)
